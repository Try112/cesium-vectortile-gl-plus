...
function getLineLabelCoordinate(coordinates) {
  if (!coordinates?.length) return null
  return coordinates[Math.floor(coordinates.length / 2)]
}

function getPolygonLabelCoordinate(rings) {
  const ring = rings?.[0]
  ...
  addLayer(features, layer, frameState, tileset) {
    ...
    for (const sourceFeature of features) {
      ...
      if (geometryType == 'Point') {
        ...
        const coord = getLineLabelCoordinate(coordinates)
        if (coord) {
          addText(
            coord,
            text,
        ...
        for (const line of coordinates) {
          const coord = getLineLabelCoordinate(line)
          if (coord) {
            addText(
              coord,
              text,
        ...
        const coord = getPolygonLabelCoordinate(coordinates)
        if (coord) {
          addText(
            coord,
            text,
        ...
        for (const polygon of coordinates) {
          const coord = getPolygonLabelCoordinate(polygon)
          if (coord) {
            addText(
              coord,
              text,
        ...
        warnOnce('symbol图层：不支持该几何类型 ' + geometryType)
      }
    }
